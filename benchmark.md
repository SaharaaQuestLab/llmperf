...
```bash
docker run -d --runtime nvidia --gpus "device=0" --rm --name Llama-3.1-70B-Instruct -v /data0/huggingface:/root/.cache/huggingface --env HUGGING_FACE_HUB_TOKEN=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 -p 80:8000 --ipc=host vllm/vllm-openai:v0.5.3 --model meta-llama/Llama-3.1-70B-Instruct --gpu-memory-utilization 0.9  --max-model-len 100000
